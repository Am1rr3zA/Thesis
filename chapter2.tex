...
Another purpose of this example is to show that you can reach the same level of detection by combining very simple and naive model (detector) instead of spending time to find the proper model and wait to train that model on data set, which is usually the time consuming task. In this example, hundreds of linear detector (which is very fast, but it's not accurate) are trained over a data set. In order to have diverse learner detectors we changed the ratio of data and their label during training to get as diverse detectors as possible.

As shown below, we have different trained detectors on these data sets which none of them can detect all the classed but as we will show later we can use them to better classify the data set by combining these simple detectors.
...
By selecting the right models and combine them together we can increase the detection rate, as below plots show; the plot on the left show all the original detectors and tehir ROC before combination, after selecting the proper detectors and combine them together (after just one iteration) we can reach the right plot which shows the combined detectors (in yellow) and their improved ROC curve, which shows more accurate result. The next step is to show why combination proivdes better result and how we can get the same result just with combining part of trained models.
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{ROC comparison on Lithuanian dataset before and after first round of combination}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{ROC comparison on Banana dataset before and after first round of combination}
...
\begin{figure}[H]
    ...
    \end{subfigure}
    \caption{ROC comparison on Circle dataset before and after first round of combination}
...
\caption{Some of best LDA ( Linear Discriminant Analysis) that can be trained on Lithuanina data set} \label{fig:Lithuanian_all_single}
\end{figure}

As it's shown in Figure~\ref{fig:Lithuanian_all_single} with just one LDA we cannot classify all the classes 100\%. ‌But with closer look at this data set and our LDA we can see that it's possible to choose two of these detectors and combine them and get a better result. As it's shown in Figure~\ref{fig:Dataset_ROC_Lithuanian} we select detector from Firgure~\ref{fig:Lithuanian_all_single_a}  and detector form ~\ref{fig:Lithuanian_all_single_d} and combine them together.
...
If we combine these two detectors with help of $AND$ operation as you can see the result in Figure~\ref{fig::combined_lithuanian_roc}  we can get a better result. The big red point is the mapping of the combined detectors, and it's clear and it's improved since it's passed the current ROC and if we recalculate the ROC based on this new generated point we will have greater AUC.
...
The Cricle data set is a good example that doing combination in an iterative way will improve the result, as it is shown in figure~\ref{fig:Circle_all_single} There are not any two detectors that can be selected and combined together in order to classify the whole data set completely, yet if we select all of these good detectors and start by combining two of them and for next iteration combine the result with the next one as you can see in figure~\ref{fig::combined_circle_roc} we can classify the whole data set with the help of few simple detectors.
...
\caption{Series of 2 selected LDA ( Linear Discriminant Analysis) that can be trained and combined on Circle data set} \label{fig:Circle_all_single}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[scale=0.6]{figs/Circle/9999999-combined_selected_before_combination}
\caption{Best potential selected detectors on Circle data set}
